...
<form id="inputForm" method="post" action="update">
	<input type="hidden" id="infoId" name="id" value="${program.id}" />
       ...
       <div class="pop_grouprmation_mod">
            <ul class="pop_list merchant_type_add">
                	...
                	<li class="clearfix">
                		...
                		<input class="c_input_text required" type="text" style="width:200px;" name="title" id="title" value="${(program.title)!''}" realValue="请输入节目名称" maxlength="32" />
               		</li>
               		
               		<li class="clearfix">
                		<label for="sort require digits" class="tit">排序：<span class=" red">*</span></label>
                		<input class="c_input_text required digits" type="text" style="width:200px;" name="sort" id="sort" value="${(program.sort)!''}" realValue="请输入节目排序" maxlength="32" />
               		</li>
               		...
               		<li class="clearfix">
                        ...
                        <br/>
                       	<input class="c_input_text" type="hidden" id="titleImage" value="${(program.titleImage)!''}"  name="titleImage"/>
                	...
                	<li class="clearfix">
                		<label for="state" class="tit">状态：<span class=" red">*</span></label>
               			<input type="radio" name="state" value="1" <#if program.state ==1 > checked="checked" <#else> </#if> />启用
                		<input type="radio" name="state" value="0" <#if program.state ==0 > checked="checked" <#else>  </#if> />停用
                	</li>            
...
<script type="text/javascript">
	...
	$inputForm.validate({
		rules: {
			title:{
				remote: {
	        		url: "/program/repeat-title?id="+$("#infoId").val()
	        	}
	        }
		},
		messages: {
